...
async function fireAlert(alert, context) {
  ...
  fireWebhooks('alert.triggered', { alert_name: alert.name, condition: alert.condition, message: context.message });
...
// Expire timed-out HITL requests
async function expireHitlRequests() {
  try {
    await pool.query(
