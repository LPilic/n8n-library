...
const { n8nApiFetch, getWorkflowNameMap } = require('./n8n-api');
...
async function aiComplete(systemPrompt, userPrompt, maxTokens = 1024) {
  ...
  if (provider === 'ollama') {
    ...
    if (baseUrl && !validateExternalUrl(baseUrl)) throw new Error('Invalid Ollama base URL');
...
async function generateDailySummary() {
  const [data, wfMap] = await Promise.all([
    n8nApiFetch('/api/v1/executions?limit=250'),
    getWorkflowNameMap(),
  ]);
  ...
  const wfName = (ex) => wfMap[ex.workflowId] || ex.workflowId || 'Unknown';
  ...
  const withDuration = executions
    ...
    .map(e => ({ name: wfName(e), duration: new Date(e.stoppedAt) - new Date(e.startedAt) }))
