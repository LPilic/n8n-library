...
let disabledToolsCache = { value: [], ts: 0 };
const DISABLED_TOOLS_TTL = 30000; // 30 seconds

async function getDisabledTools() {
  if (Date.now() - disabledToolsCache.ts < DISABLED_TOOLS_TTL) {
    return disabledToolsCache.value;
  }
  try {
    const { rows } = await pool.query("SELECT value FROM settings WHERE key = 'mcp_disabled_tools'");
    const value = rows.length ? JSON.parse(rows[0].value) : [];
    disabledToolsCache = { value, ts: Date.now() };
    return value;
  } catch { return disabledToolsCache.value; }
}
