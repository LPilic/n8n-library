...
async function getWorkflowNameMap(instanceId) {
  ...
  if (cached && cached.map && Object.keys(cached.map).length > 0 && Date.now() - cached.ts < 300000) { // 5 min TTL
    return cached.map;
  }
  try {
...
// Shared helper used by multiple route files — resolves instance base URL from request
